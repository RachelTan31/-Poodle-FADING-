...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        if self.counter > self.start_blend:
            # --- SAFETY GUARDS FOR NEWER DIFFUSERS VERSIONS ---
            # If these keys are missing (or lists too short), skip local blending
            # instead of crashing with KeyError / index errors.
            if ("down_cross" not in attention_store) or ("up_cross" not in attention_store):
                return x_t

            down_list = attention_store["down_cross"]
            up_list = attention_store["up_cross"]

            # Original code assumes at least 4 down_cross and 3 up_cross entries
            if len(down_list) < 4 or len(up_list) < 3:
                return x_t

            maps = down_list[2:4] + up_list[:3]
